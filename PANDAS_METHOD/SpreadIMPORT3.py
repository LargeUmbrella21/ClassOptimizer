...
def optimize_schedule():
   ...
   # Sanitize class names to avoid issues with spaces or special characters
    sanitized_classes = {name.replace(" ", "_").strip(): cap for name, cap in classes.items()}
    sanitized_preferences = {
        student: {c.replace(" ", "_").strip(): rank for c, rank in prefs.items()}
        for student, prefs in preferences.items()
    }
    sanitized_availabilities = {
        student: [c.replace(" ", "_").strip() for c in avail_list]
        for student, avail_list in availabilities.items()
    }
    ...
    weight_fill = 50
    rankings = {}
    for student, ranks in sanitized_preferences.items():
        for class_name in sanitized_classes:
            if class_name in ranks:
                rankings[(student, class_name)] = ranks[class_name]
            elif class_name in sanitized_availabilities.get(student, []):
    ...
    print("\n\U0001F4CC Student Rankings for Classes:")
    ...
    x = LpVariable.dicts("assign", [(s, c) for s in students for c in sanitized_classes], cat="Binary")
    unfilled_penalty = LpVariable.dicts("unfilled", sanitized_classes.keys(), lowBound=0, cat="Continuous")

    # Objective function
    problem += (
        lpSum(rankings[(s, c)] * x[(s, c)] for s in students for c in sanitized_classes if rankings[(s, c)] != float('inf'))
        + weight_fill * lpSum(unfilled_penalty[c] for c in sanitized_classes),
        "TotalDissatisfaction"
    )

    # Constraints: Each student gets one class
    for s in students:
        problem += lpSum(x[(s, c)] for c in sanitized_classes if rankings[(s, c)] != float('inf')) == 1, f"OneClassPerStudent_{s}"

    # Constraints: Class capacities
    for c in sanitized_classes:
        assigned_students = lpSum(x[(s, c)] for s in students if rankings[(s, c)] != float('inf'))
        problem += assigned_students <= sanitized_classes[c], f"ClassCapacity_{c}"
        problem += unfilled_penalty[c] >= (sanitized_classes[c] - assigned_students), f"UnfilledPenalty_{c}"
    ...
    unfilled_classes = {c: unfilled_penalty[c.replace(" ", "_").strip()].value() for c in classes if unfilled_penalty[c.replace(" ", "_").strip()].value() > 0}
    ...
    for s in students:
        for c in sanitized_classes:
         if x.get((s, c)) and x[(s, c)].value() == 1:

            sanitized_c = c.replace(" ", "_").strip()
            if x.get((s, sanitized_c)) and x[(s, sanitized_c)].value() == 1:
